...
vim.keymap.set('n', '<leader>cr', function() utils.build_and_run() end, { desc = '[C++] Compile & Run', buffer = true })
    ...
    vim.fn.termopen({ 'bash', '-c', build_cmd }, {
        on_exit = function(_, code)
            vim.schedule(function()
                if code == 0 and vim.api.nvim_win_is_valid(win) then vim.api.nvim_win_close(win, true) end
            end)
        end,
    })
...
-- CMake shortcuts
vim.keymap.set('n', '<leader>cC', function() utils.cmake_build { type = 'Debug' } end, { desc = '[C++] CMake build (Debug)', buffer = true })

vim.keymap.set('n', '<leader>cO', function() utils.cmake_build { type = 'Release' } end, { desc = '[C++] CMake build (Release)', buffer = true })
