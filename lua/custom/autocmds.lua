...
local function is_autosaveable(buf) return vim.bo[buf].modified and vim.bo[buf].buftype == '' and vim.api.nvim_buf_get_name(buf) ~= '' end
...
vim.api.nvim_create_autocmd('FocusLost', {
    ...
    callback = function()
        for _, buf in ipairs(vim.api.nvim_list_bufs()) do
            if is_autosaveable(buf) then vim.api.nvim_buf_call(buf, function() vim.cmd 'silent! write' end) end
        end
    end,
})
...
vim.api.nvim_create_autocmd('BufLeave', {
    ...
    callback = function(args)
        if is_autosaveable(args.buf) then vim.api.nvim_buf_call(args.buf, function() vim.cmd 'silent! write' end) end
    end,
})
