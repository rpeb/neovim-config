...
function M.build_and_run(opts)
  ...
  local cmd = string.format(
    'cd "%s" && %s %s %s %s "%s" -o "%s" && ./%s %s; rm -f "%s"',
    dir, compiler, std, flags, debug_flags, file, exe, exe, run_args, exe
  )
...
function M.build_for_debug()
  ...
  local cmd = string.format(
    'cd "%s" && %s %s -g -Wall -Wextra "%s" -o "%s" && echo "Build OK: ./%s"',
    dir, compiler, std, file, exe, exe
  )
...
function M.cmake_build(opts)
  ...
  local cmd = string.format(
    'cmake -B build -DCMAKE_BUILD_TYPE=%s -DCMAKE_EXPORT_COMPILE_COMMANDS=ON && cmake --build build -j%d',
    build_type, jobs
  )
...
function M.set_buffer_opts()
  ...
  vim.opt_local.makeprg = vim.bo.filetype == 'cpp'
    and 'g++ -std=c++20 -Wall -Wextra %:t -o %:t:r'
    or 'gcc -std=c11 -Wall -Wextra %:t -o %:t:r'
end
